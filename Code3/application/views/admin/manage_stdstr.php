	 ...
	 if(a)
     {
							...
							$.ajax({
									...
									success:function(result)
									{
										swal({
											title: "Deleted!",
											text: "Student Strength Details Deleted Successfully!",
											icon: "success"
										}).then(function() {
...
function updates(id)
{
					...
					$.ajax({
									...
									{
										console.log(result);
										 window.location.href = "updateforms";
									}
								});
...
<h3 class="page-header"><center>Student Strength Details</center></h3>
...
<?php
	...
	foreach ($data as $row)
	{
		...
		foreach ($dept as $col)
		{
			echo "<td>".$col->Dept_name."</td>";
		}
		echo "<td>".$row->Pgm_name."</td>";
		echo "<td>".$row->Sem."</td>";
		echo "<td>".$row->Std_strength."</td>";
		
		
		
		echo "<td class='text-center'><a href='#' onclick='updates($row->sl_no)'><span class='glyphicon glyphicon-pencil'style=color:green;></span></a></td>";
		
		
		echo "<td class='text-center'><a href='#' onclick='deletes($row->sl_no)'><span class='glyphicon glyphicon-remove' style=color:red;></span></a></td>";
