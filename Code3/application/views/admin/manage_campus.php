	 ...
	 if(a)
     {
							...
							$.ajax({
									...
									success:function(result)
									{
										swal({
											title: "Deleted!",
											text: "Campus Details Deleted Successfully!",
											icon: "success"
										}).then(function() {
...
function updates(id)
{
					...
					$.ajax({
									...
									{
										console.log(result);
										 window.location.href = "updateformc";
									}
								});
...
<h3 class="page-header"><center>Campus Details</center></h3>
...
<?php
	...
	foreach ($data as $row)
	{
		...
		echo "<td class='text-center'><a href='#' onclick='updates($row->Campus_code)'><span class='glyphicon glyphicon-pencil'style=color:green;></span></a></td>";
		
		
		echo "<td class='text-center'><a href='#' onclick='deletes($row->Campus_code)'><span class='glyphicon glyphicon-remove' style=color:red;></span></a></td>";
