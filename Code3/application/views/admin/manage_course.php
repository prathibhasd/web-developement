	 ...
	 if(a)
     {
							...
							$.ajax({
									...
									success:function(result)
									{
										swal({
											title: "Deleted!",
											text: "Course Details Deleted Successfully!",
											icon: "success"
										}).then(function() {
...
function updates(id)
{
					...
					$.ajax({
									...
									{
										console.log(result);
										 window.location.href = "updatefcourse";
									}
								});
...
<h3 class="page-header"><center>Course Details</center></h3>
...
<?php
	...
	foreach ($data as $row)
	{
		...
		echo "<td class='text-center'><a href='#' onclick='updates($row->C_no)'><span class='glyphicon glyphicon-pencil'style=color:green;></span></a></td>";
		
		
		echo "<td class='text-center'><a href='#' onclick='deletes($row->C_no)'><span class='glyphicon glyphicon-remove' style=color:red;></span></a></td>";
